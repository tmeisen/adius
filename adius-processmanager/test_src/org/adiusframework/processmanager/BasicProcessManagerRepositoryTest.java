...
import org.adiusframework.processmanager.domain.dao.SimulationProcessDao;
import org.adiusframework.processmanager.domain.dao.SimulationStepDao;
...
public class BasicProcessManagerRepositoryTest {
	...
	private static final String TEST_STEP_ID = "testStep";
	...
	// The Data-Access-Objects, which interact with the BasicProcessManagerRepository
	private SimulationProcessDao processDao;
	private SimulationStepDao stepDao;
	...
	@Before
	public void init() {
		processDao = mock(SimulationProcessDao.class);
		stepDao = mock(SimulationStepDao.class);
		taskDao = mock(ServiceTaskDao.class);
		
		// Create and initialize the test-object
		repository = new BasicProcessManagerRepository();
		repository.setSimulationProcessDao(processDao);
		repository.setSimulationStepDao(stepDao);
		repository.setServiceTaskDao(taskDao);
	}
	
	@Test
	public void testCreateSimulationProcess() {
		// Call the tested method with our test-parameters
		SimulationProcess process = repository.createSimulationProcess(TEST_PROCESS_ID, TEST_STEP_ID, TEST_DOMAIN);
		
		// And verify if the SimulationProcess and the SimulationStep are created correctly
		assertEquals(process.getIdentifier(), TEST_PROCESS_ID);
		assertEquals(process.getDomain(), TEST_DOMAIN);
		assertEquals(1, process.getSteps().size());
		SimulationStep step = process.getSteps().values().iterator().next();
		assertEquals(step.getIdentifier(), TEST_STEP_ID);
		assertEquals(step.getProcess(), process);
		verify(processDao, times(1)).save(process);
		verify(stepDao, times(1)).save(step);
	}
	
	@Test
	public void testExtendTaskByServiceProcessPlan() {
		// Create a ServiceProcessPlan with it's related ServiceTemplate, Properties and ServiceTask
