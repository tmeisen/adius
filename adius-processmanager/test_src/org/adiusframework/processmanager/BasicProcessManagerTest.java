...
public class BasicProcessManagerTest {
	...
	// Related objects, which's methods could by called by the ProcessManager in order to achieve the result
	...
	public void init() throws ProcessManagerException {
		...
		when(executionEngine.handleQuery(query)).thenReturn(TEST_INTERNAL_ID);
		...
		// The BasicProcessManager should register as a listener to the ExecutionEngine-mock
	...
	public void testHandleQuery() throws ProcessManagerException {
		...
		verify(executionEngine, times(1)).handleQuery(query);
		verify(callbackRepository, times(1)).register(TEST_INTERNAL_ID, callback);
	}
	
	@Test
	public void testHandleQueryProcessManagerException() throws ProcessManagerException {
		when(executionEngine.handleQuery(query)).thenThrow(exception);
