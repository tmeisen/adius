...
public class MatrixSTN implements SimpleTemporalNetwork {
	...
	public TimeStampedPlan getTimes() {
		...
		while (ait.hasNext()) {
			...
			if (a instanceof StartInstantAction) {
				...
				BigDecimal dur = TheArray[Timepoints.indexOf(da.endAction)][0].negate().subtract(time)
	...
	public void maximize(DurativeAction da) {
		TheArray[Timepoints.indexOf(da.endAction)][Timepoints.indexOf(da.startAction)] = TheArray[Timepoints
				.indexOf(da.startAction)][Timepoints.indexOf(da.endAction)].negate();
	}

	public void minimize(DurativeAction da) {
		TheArray[Timepoints.indexOf(da.startAction)][Timepoints.indexOf(da.endAction)] = TheArray[Timepoints
				.indexOf(da.endAction)][Timepoints.indexOf(da.startAction)].negate();
	}
