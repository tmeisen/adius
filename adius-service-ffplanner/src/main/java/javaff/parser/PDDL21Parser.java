...
public class PDDL21Parser implements PDDL21ParserConstants {
	private static final Logger LOGGER = LoggerFactory.getLogger(PDDL21Parser.class);

	private UngroundProblem up;

	public static UngroundProblem parseStrings(String pDomainString, String pProblemString) {
		UngroundProblem up = new UngroundProblem();
		boolean probsuc = false;
		boolean suc = parseDomainString(up, pDomainString);
		if (suc)
			probsuc = parseProblemString(up, pProblemString);
		if (probsuc) {
	...
	public static UngroundProblem parseFiles(File pDomainFile, File pProblemFile) {
		UngroundProblem up = new UngroundProblem();
		boolean probsuc = false;
		boolean suc = parseDomainFile(up, pDomainFile);
		if (suc)
			probsuc = parseProblemFile(up, pProblemFile);
		if (probsuc) {
			// ADL quantifiers (Exists and ForAll) need to have their quantified
			// variables set up after the
			// problem file has been parsed. The problem file cannot be swapped
	...
	private static boolean parseDomainString(UngroundProblem up, String pDomainString) {
		boolean req = false;
		try {
			StringReader domainReader = new StringReader(pDomainString);
			PDDL21Parser parser = new PDDL21Parser(up, domainReader);
			req = parser.parseDomain();
			domainReader.close();
		} catch (ParseException e) {
			LOGGER.error("Error whilst parsing string domain");
			LOGGER.error(e.getMessage());
			req = false;
		}
		if (req)
			LOGGER.debug("Parsed domain string successfully");
		else
			LOGGER.debug("Parsing of domain string failed");
		return req;
	}

	private static boolean parseDomainFile(UngroundProblem up, File pFile) {
		boolean req = false;
		try {
			FileReader tFileReader = new FileReader(pFile);
			PDDL21Parser parser = new PDDL21Parser(up, tFileReader);
			req = parser.parseDomain();
			tFileReader.close();
		} catch (FileNotFoundException e) {
			LOGGER.error("File " + pFile + " has not been found");
			req = false;
		} catch (IOException e) {
			...
			LOGGER.error("Error whilst parsing file " + pFile);
			LOGGER.error(e.getMessage());
			req = false;
		}
		if (req)
			LOGGER.debug("Parsed Domain file " + pFile + " successfully");
		else
			LOGGER.debug("Parsing of Domain file " + pFile + " failed");
		return req;
	}

	private static boolean parseProblemString(UngroundProblem up, String pProblemString) {
		boolean probsuc = true;
		try {
			StringReader problemReader = new StringReader(pProblemString);
			PDDL21Parser parser = new PDDL21Parser(up, problemReader);
			parser.parseProblem();
			problemReader.close();
		} catch (ParseException e) {
			LOGGER.error("Error whilst parsing file " + pProblemString);
			LOGGER.error(e.getMessage());
			probsuc = false;
		}
		if (probsuc)
			LOGGER.debug("Parsed problem string successfully");
		else
			LOGGER.debug("Parsing of problem string failed");
		return probsuc;
	}

	private static boolean parseProblemFile(UngroundProblem up, File pFile) {
		boolean probsuc = true;
		try {
			FileReader tFileReader = new FileReader(pFile);
			PDDL21Parser parser = new PDDL21Parser(up, tFileReader);
		...
		if (probsuc)
			LOGGER.debug("Parsed Problem file " + pFile + " successfully");
		else
			LOGGER.debug("Parsing Problem file " + pFile + " failed");
		return probsuc;
	}

	private class TypedList {
		public List<String> objects = new ArrayList<String>();
		public Map<String, Type> types = new Hashtable<String, Type>();

		public void add(String s) {
			objects.add(s);
			types.put(s, Type.rootType);
		}
	...
	final public boolean parseRequireDef() throws ParseException {
		...
		label_2: while (true) {
			...
			if (jj_2_8(20)) {
				...
				System.out.println("cannnot parse " + t.toString().toLowerCase());
				{
					if (true)
						return false;
				}
	...
	// ***************************************************
	// * Parse a TypedList recursive Version
	// ***************************************************
	/*
	 * final public TypedList parseTypedList() throws ParseException { List
	 * strings = new ArrayList(); Type type; Token t; TypedList rList = new
	 * TypedList(); TypedList tList; if (jj_2_22(20)) { label_7: while (true) {
	 * if (jj_2_16(20)) { t = jj_consume_token(NAME); } else if (jj_2_17(20)) {
	 * t = jj_consume_token(VAR); } else { jj_consume_token(-1); throw new
	 * ParseException(); } strings.add(t.toString().toLowerCase()); if
	 * (jj_2_18(20)) { ; } else { break label_7; } }
	 * jj_consume_token(TYPE_IDENT); type = parseType();
	 * //LOGGER.info("PDDLPARSER "); Iterator tit = strings.iterator(); while
	 * (tit.hasNext()) { String string = (String) tit.next();
	 * //LOGGER.info("PDDLPARSER rList " + string + " Type " + type);
	 * rList.add(string, type); } tList = parseTypedList();
	 * rList.objects.addAll(tList.objects); rList.types.putAll(tList.types); }
	 * else { label_8: while (true) { if (jj_2_19(20)) { ; } else { break
	 * label_8; } if (jj_2_20(20)) { t = jj_consume_token(NAME); } else if
	 * (jj_2_21(20)) { t = jj_consume_token(VAR); } else { jj_consume_token(-1);
	 * throw new ParseException(); } rList.add(t.toString().toLowerCase()); } }
	 * { if (true) return rList; } throw new
	 * Error("Missing return statement in function"); }
	 */

	// ***************************************************
	// * Parse a TypedList iterative Version
	// ***************************************************
	final public TypedList parseTypedList() throws ParseException {
		...
		label_9: while (true) {
			...
			if (jj_2_22(20)) {
				...
				jj_consume_token(TYPE_IDENT);
				type = parseType();
				// LOGGER.info("PDDLPARSER ");
				Iterator tit = strings.iterator();
				// tList = new TypedList();
				while (tit.hasNext()) {
					String string = (String) tit.next();
					// LOGGER.info("PDDLPARSER rList " + string + " Type " +
					// type);
					tList.add(string, type);
				}
				...
				/*
				 * if no element is left, leave the loop
				 */
				if (!addElement) {
					break label_9;
				}
	...
	final public PredicateSymbol parsePredicateSymbol() throws ParseException {
		Token t;
		t = jj_consume_token(NAME);
		PredicateSymbol ps = up.predSymbolMap.get(t.toString().toLowerCase());
		if (ps == null) {
	...
	final public FunctionSymbol parseFunctionSymbol() throws ParseException {
		Token t;
		t = jj_consume_token(NAME);
		FunctionSymbol fs = up.funcSymbolMap.get(t.toString().toLowerCase());
		if (fs == null) {
	...
	final public SingleLiteral parseLiteralName() throws ParseException {
		SingleLiteral pp;
	...
	public PDDL21Parser(UngroundProblem up, java.io.InputStream stream) {
		this(up, stream, null);
	}

	/** Constructor with InputStream and supplied encoding */
	public PDDL21Parser(UngroundProblem up, java.io.InputStream stream, String encoding) {
		this.up = up;
		try {
	...
	/** Constructor. */
	public PDDL21Parser(UngroundProblem up, java.io.Reader stream) {
		this.up = up;
	...
	public PDDL21Parser(UngroundProblem up, PDDL21parserTokenManager tm) {
		this.up = up;
		token_source = tm;
		token = new Token();
		jj_ntk = -1;
