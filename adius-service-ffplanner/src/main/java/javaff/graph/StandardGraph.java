...
public class StandardGraph<V, E> extends DirectedMultigraph<V, E> {
	...
	/**
	 * Simple check which permits edges to exist. Default implementation only
	 * allows 1 edge between nodes.
	 * 
	 * @param start
	 * @param end
	 */
	...
	public void generateDotGraph(File dotFile) {
		...
		try {
			...
			for (E e : this.edgeSet()) {
				...
				// Iterator<RelationshipEdge> outEdgesIter =
				// this.graph.outgoingEdgesOf(p).iterator();
			...
			// Process p =
			// Runtime.getRuntime().exec("dot -Tpng \'"+dotFile.getAbsolutePath()+"_dot\' > \'test.png/'");
			// p.waitFor();
