...
public class RelaxedMetricPlanningGraph extends RelaxedPlanningGraph {
	...
	public void setGoal(GroundFact g) {
		// hack to get around fact this constructor calls up to PlaningGraph,
		// which calls this overridden
		// method, which in turn will require that PGFuncMap has been populated-
	...
	protected class PGBinaryComparator {
		...
		public boolean met(int layer, List maxes, List mins) {
			if (type == MetricSymbolStore.GREATER_THAN)
				...
				return (left.getMaxValue(layer, maxes, mins).compareTo(right.getMinValue(layer, maxes, mins)) >= 0 && left
						.getMinValue(layer, maxes, mins).compareTo(right.getMaxValue(layer, maxes, mins)) <= 0);
			else
				return true;
		}
