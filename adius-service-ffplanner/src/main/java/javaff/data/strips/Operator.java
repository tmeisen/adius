...
public abstract class Operator implements javaff.data.PDDLPrintable {
	...
	public List<Variable> params = new ArrayList<Variable>(); // list of
																// Variables
	...
	public Set<Action> ground(UngroundProblem up) {
		...
		out: for (ArrayList<PDDLObject> l : s) {
			...
			// 17/3/11 -- Bit of a hack to get the ADL stuff working. Need to
			// pass in
			// specific parameter types along with normal groundings. Only way
			// to
			// find out if these are required is to check the actions effects
			// and conditions

			Action groundedAction = ground(l); // ground an individual action
												// using the specified grounded
												// params

			// 23/8/11 -- Another hack to eliminate any grounded actions whose
			// preconditions contain
			// a static fact which is not true in the initial state, rendering
			// it impossible
	...
	public Set<ArrayList<PDDLObject>> getParameterCombinations(UngroundProblem up) {
		...
		for (Fact fp : staticConditions) {
			...
			Set<Proposition> sp = up.staticPropositionMap.get(p.getPredicateSymbol());
			...
			for (Proposition prop : sp) {
				for (ArrayList<PDDLObject> c : combinations) {
					...
					while (propargit.hasNext() && ok) {
						...
						Parameter k = p.getParameters().get(counter);
		...
		for (Variable p : params) {
			...
			if (!set[counter]) {
				...
				for (ArrayList<PDDLObject> s : combinations) {
					Set<PDDLObject> objs = up.typeSets.get(p.getType());
