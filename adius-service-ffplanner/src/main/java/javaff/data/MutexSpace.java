...
public class MutexSpace implements Cloneable {
	...
	/**
	 * Gets the set of keys within this mutex set. Keys are facts which have a
	 * known mutex set.
	 * 
	 * @return
	 */
	...
	/**
	 * Add the mutex to the set. Note that this will overwrite existing mutexes
	 * for the parameter's "owner" property.
	 * 
	 * @param mutex
	 */
	...
	/**
	 * Creates a mutex for all facts in the set specified, such that f is mutex
	 * with (facts \ f) forall f in facts
	 * 
	 * @param mutex
	 */
	public void addMutex(Collection<Fact> facts) {
		for (Fact f : facts) {
	 ...
	 *             Thrown if the first parameter does not exist in this goal
	 *             space.
	 */
	...
	/**
	 * Explicitely constructs a set of mutexes when called. Can be expensive on
	 * large problems.
	 * 
	 * @return
	 */
	...
	public Object clone() {
		...
		newThis.mutexMap = new HashMap<Fact, FactMutex>(this.mutexMap);
