...
public class EnforcedHillClimbingSearch extends Search {
	private static final Logger LOGGER = LoggerFactory.getLogger(EnforcedHillClimbingSearch.class);
	...
	public State search() {
		...
		while (!open.isEmpty()) // whilst still states to consider
		{
			...
			Set filterActions = filter.getActions(s);
			Set successors = s.getNextStates(filterActions); // and find
